...
const TypenameSet = std.StringHashMap(void);
const debug = true;
...
fn genNativeUnion(
    ...
    while (i < e.ValuesLen()) : (i += 1) {
        ...
        _ = try writer.write(": ");
        try genTypeBasic(ev.UnionType().?, schema, .keep_namespace, imports, writer);
        _ = try writer.write(",\n");
    }
}
...
fn genNativeUnionUnpack(e: Enum, schema: Schema, imports: *TypenameSet, writer: anytype) !void {
    ...
    while (i < e.ValuesLen()) : (i += 1) {
        ...
        if (ev.Value() == 0)
            ...
            try genTypeBasic(ev.UnionType().?, schema, .skip_namespace, imports, writer);
            try writer.print(
...
fn genPrelude(
    ...
    while (iter.next()) |it| {
        ...
        if (mem.eql(u8, name, typename)) continue;
        const path = try typenameToPath(&buf, "", name, ".fb.zig");
        const lastname = lastName(name);
        try writer.print(
            \\const {0s} = @import("{1s}").{0s};
            \\
        , .{ lastname, path });
    }
}
    ...
    switch (field_base_ty) {
        ...
        .STRUCT, .VECTOR => _ = try writer.write("0"), // NOTE: not sure about these values, just copied go ouptut
            ...
            if (field_ty.Index() == -1)
                ...
                // std.debug.print("field_ty.BaseType()={}\n", .{field_ty.BaseType()});
                if (field_base_ty == .UTYPE) {
...
fn getStructFieldOfStruct(o: Object, field: Field, writer: anytype) !void {
    ...
    try writer.print(
        \\pub fn {0s}(rcv: {1s}) {1s} {{
        \\return {1s}.init(rcv._tab.bytes, rcv._tab.pos + {2});
        \\}}
        \\
        \\
    , .{ fname_camel_upper, oname, field.Offset() });
}
...
fn buildVectorOfTable(_: Object, field: Field, schema: Schema, writer: anytype) !void {
    ...
    try writer.print(
        \\pub fn Start{s}Vector(b: *Builder, num_elems: i32) !u32 {{
